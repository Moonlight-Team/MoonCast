...
func _ground_state_2D(player:MoonCastPlayer2D) -> void:
	if Input.is_action_pressed(player.button_down) and is_zero_approx(player.ground_velocity):
		player.can_jump = false
		if Input.is_action_pressed(player.button_jump):
			player.play_animation(&"spindash", true)
			if not spindashing:
				spindash_total = spindash_inital
				spindashing = true
			else:
				spindash_total += spindash_accumulate
	else:
